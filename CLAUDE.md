...
- Manual-trigger skills (`disable-model-invocation: true`): `/spec` (refine vague ideas) and `/release` (cut a Homebrew release tag for the `ralph` CLI — repo-maintainer only, not included in `ralph init`). All others (plan, work, loop, self-review, verify, test, codex-review, pr, sync-docs, audit-harness) are auto-invoked.
