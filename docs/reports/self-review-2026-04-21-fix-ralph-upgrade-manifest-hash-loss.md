...
- Date: 2026-04-21
- Trigger: Re-review after Round 4 Codex findings (Finding 6: heal-loop; Finding 7: reintroduction overwrite) landed in commit `ef8e3ed`.
- Scope: `git show ef8e3ed` only — `internal/upgrade/diff.go` (+28/−6), `internal/upgrade/diff_test.go` (+60/−2), `docs/reports/codex-triage-*.md` (+14, out of code-quality scope). All prior commits approved in Rounds 1–4.
- Reviewer: reviewer subagent (self-review, diff quality only).
- Focus (per /self-review invocation): (1) heal-conflict `OldHash=newHash` change and (2) `ActionAdd` disk-peek safeguard promoting to `ActionConflict` when disk differs.

### What the commit does

1. **Heal-conflict `OldHash=newHash` (diff.go:131-148)** — in the empty-hash + user-edit branch, the emitted `ActionConflict` previously carried `OldHash=""` (the manifest's own zero-value hash). On non-interactive skip (`runUpgrade` writes `d.OldHash` to the manifest at `upgrade.go:217`), the corruption was re-persisted and the file re-entered the heal branch on every subsequent same-version run. The fix sets `OldHash: newHash` so skip adopts the template as the new baseline; the next run matches `newHash == mf.Hash` and falls into `ActionSkip` without re-prompting.
2. **`ActionAdd` disk-peek safeguard (diff.go:76-105)** — `diskHash` / `diskErr` are computed up front (moved from the previously lower position inside the `inManifest` branch) so the `!inManifest` branch can distinguish a safe add (disk missing or already matches template) from a potentially overwriting add (disk has different content). When disk differs, the walk emits `ActionConflict` instead of `ActionAdd`, forcing an interactive prompt. This closes a silent-overwrite on the "template-removed-then-reintroduced" reintroduction sequence.
3. **Tests** — strengthens `TestComputeDiffs_EmptyHashConflictsWhenDiskDiffers` to assert `OldHash == newHash` post-heal, and adds `TestComputeDiffs_AddBecomesConflictWhenDiskDiffers` (reintroduction → conflict) plus `TestComputeDiffs_AddStaysAddWhenDiskMatchesTemplate` (no-op re-add stays safe).

### Findings

| Severity | Area | Finding | Evidence | Recommendation |
| --- | --- | --- | --- | --- |
| MEDIUM | `internal/upgrade/diff.go:88-97` — reintroduction `ActionConflict` has `OldHash=""` | The new `!inManifest + disk differs` branch emits `ActionConflict` **without setting `OldHash`**, so the zero-value `""` flows through. The `runUpgrade` skip resolution at `internal/cli/upgrade.go:214-218` writes `manifest.SetFile(d.Path, d.OldHash)` — i.e., writes an empty hash into the fresh manifest. On the next upgrade this entry then enters the empty-hash heal branch at `diff.go:123`, where the already-fixed heal-conflict path (Finding 6) handles it correctly. Net effect: convergence takes **two upgrade runs** instead of one, and the first run transiently introduces the very empty-hash corruption pattern that Round 4 is supposed to prevent. The two fixes in this commit are siblings; the reintroduction-conflict branch should set `OldHash: newHash` for exactly the same reason the heal-conflict branch does. | `diff.go:88-97` — `FileDiff{Path, Action: ActionConflict, DiskHash, NewHash, NewContent}` — no `OldHash` field. `upgrade.go:214-218` — `case "skip": manifest.SetFile(d.Path, d.OldHash)` writes `""` when the field is zero-valued. Contrast with `diff.go:140-147` (heal-conflict) which now sets `OldHash: newHash` after Finding 6. | Add `OldHash: newHash` to the reintroduction `ActionConflict` literal. One-line change. Also add a direct assertion to `TestComputeDiffs_AddBecomesConflictWhenDiskDiffers` that `diffs[0].OldHash == HashBytes(template)` (mirroring the strengthened assertion in `TestComputeDiffs_EmptyHashConflictsWhenDiskDiffers`). |
| LOW | `internal/upgrade/diff.go:81-82` — `diskHash`/`diskErr` hoisted but name scoping widens unnecessarily | Moving `diskPath`, `diskHash`, `diskErr` out to the top of the walk callback (from inside the prior `inManifest` branch) is correct for the new peek requirement, but it widens the lifetime of these locals to the entire callback. For `!inManifest && diskErr == nil && diskHash == newHash` and for the `inManifest && diskErr != nil` early-return, the hoisted variables are used. Fine. But a reader now has to track three related locals (`diskPath`, `diskHash`, `diskErr`) across ~70 lines of branching. Not a bug — the old code had the same shape in a smaller scope; the expansion is intrinsic to the feature. | `diff.go:81-82` vs pre-commit `diff.go:88-89` (inside `inManifest` branch). | Optional: consider extracting a small `peekDisk(path string) (hash string, err error)` helper if further callers appear. Not blocking today. |
| LOW | `internal/upgrade/diff.go:88` — `diskErr != nil` silently falls through to `ActionAdd` | When `os.Open` fails for a reason other than ENOENT (e.g., EACCES on the existing disk file), the code treats it as "safe to add" and proceeds to `os.WriteFile` in the caller. If write perms exist on the parent dir but the existing file is read-restricted, the user's unreadable file is silently overwritten. This is the **same pre-existing pattern** as the `inManifest` branch at `diff.go:107-115` (`diskErr != nil` → treat as add). It is not a regression from this commit — but the new peek path intentionally exists to prevent silent overwrites, so the same defensive stance arguably should apply uniformly. | `diff.go:88` (`if diskErr == nil && diskHash != newHash`) — treats `diskErr != nil` as safe-to-add. `scaffold.HashFile` at `internal/scaffold/render.go:119-131` returns `os.Open` errors unchanged; ENOENT and EACCES both produce non-nil errors. | Optional: distinguish `errors.Is(diskErr, fs.ErrNotExist)` from other errors. Other errors could be surfaced as `ActionConflict` (asking the user) rather than defaulting to `ActionAdd`. Tracks the same correctness bar the new peek was introduced to enforce. Not blocking; document as tech debt if deferring. |
| LOW | `internal/upgrade/diff_test.go:290-307` — `TestComputeDiffs_AddBecomesConflictWhenDiskDiffers` doesn't assert `OldHash` | Paired with the MEDIUM finding above: the new test asserts `diffs[0].Action != ActionConflict`, but it does NOT assert any of `OldHash` / `DiskHash` / `NewHash` / `NewContent`. If someone later changes the emit to also set `OldHash: newHash` (recommended) the test would not break, but also could not confirm the fix. Symmetry with the strengthened `EmptyHashConflictsWhenDiskDiffers` test (which now asserts `OldHash == newHash`) would catch regressions. | `diff_test.go:290-307` — only `len(diffs) != 1` and `diffs[0].Action != ActionConflict` are asserted. | Add `if diffs[0].DiskHash != HashBytes([]byte("user kept this")) || diffs[0].NewHash != HashBytes([]byte("new template version")) { ... }` and (once the MEDIUM fix lands) the `OldHash` assertion. Non-blocking. |
| LOW | `internal/upgrade/diff_test.go:309-325` — `TestComputeDiffs_AddStaysAddWhenDiskMatchesTemplate` comment prefix `"Ada:"` | The in-code comment at `diff_test.go:309` begins with `// Ada:` — looks like an accidental truncation/typo of `"Add:"` or a stray editor autocomplete. Other tests in the file use full-sentence comments (e.g., `// Regression: a file absent from the manifest...`). Cosmetic only. | `diff_test.go:309`: `// Ada: if the disk file already matches the new template, ActionAdd is safe` | Replace with `// Safe add case: if the disk file already matches ...` or similar. One-word fix. |
| LOW | `internal/upgrade/diff_test.go:307` — terminator `t.Fatalf` was previously `t.Errorf` | The strengthened `EmptyHashConflictsWhenDiskDiffers` test changes the action-mismatch assertion from `t.Errorf` (continue) to `t.Fatalf` (stop). Good defensive move — if the action is not `ActionConflict`, the subsequent `OldHash` assertion would be meaningless. No concern, just noting the intentional change. | `diff_test.go:255` (new) vs `:251` (prior `t.Errorf`). | None. |

No CRITICAL or HIGH findings.

### Correctness spot-checks

- **Heal-conflict convergence (Finding 6 fix).** Manifest `hash=""` + disk edit + template exists → `ActionConflict{OldHash: newHash, NewHash: newHash}`. Skip → `manifest.SetFile(path, newHash)`. Next run: `mf.Hash = newHash`, `newHash == mf.Hash` → `ActionSkip` at `diff.go:153-162`. Loop terminated in exactly one run, as the commit message claims. ✓
- **Overwrite resolution on heal-conflict.** User chooses overwrite → `manifest.SetFile(d.Path, d.NewHash)` at `upgrade.go:212` + disk overwritten with `NewContent`. Both disk and manifest converge to `newHash`. Correct. ✓
- **Same-version re-run semantics.** The heal-conflict comment at `diff.go:134-139` is explicit that "skip" adopts the template hash as the new baseline even though the user's edit remains on disk. This is a deliberate trade-off: the user's edit becomes untracked within this version but will be detected as a conflict when the template moves to a new version (because `diskHash != mf.Hash` and `newHash != mf.Hash`, the `diskHash == mf.Hash` check at `:165` fails → `ActionConflict`). Behavior matches the commit message. ✓
- **Reintroduction flow (Finding 7 fix).** Template v1 has `file.md` → user keeps it on disk → template v2 drops it → `ActionRemove` (entry dropped, per Round 3) → template v3 reintroduces it with new content. On v3 upgrade: `!inManifest` (Round 3 dropped the entry), disk exists with user's v1-content, `diskHash != newHash` → `ActionConflict`. User is asked. No silent overwrite. ✓
- **No-op reintroduction.** Disk matches the reintroduced template exactly → `diskHash == newHash` → `ActionAdd` → `os.WriteFile` is a no-op on content, `manifest.SetFile(d.Path, d.NewHash)` records the hash. Correct. ✓
- **ENOENT on `!inManifest`.** Template has the file, disk doesn't, not in manifest → `diskErr != nil` (ENOENT) → `ActionAdd` writes the file. Correct. ✓
- **`diskHash == newHash` on `!inManifest`.** Disk already matches template (e.g., user manually copied it) → `diskErr == nil && diskHash == newHash` → fails the `!= newHash` guard → `ActionAdd`. No conflict prompt. The ensuing `WriteFile` is a byte-identical rewrite. Correct and consistent with the existing auto-update path, which also writes even when content matches. ✓
- **Force flag interaction.** With `--force`, `resolveConflict` is bypassed and `ActionConflict` writes `NewContent` + `NewHash`. Both the heal-conflict and the reintroduction-conflict paths end up writing the template version to disk and manifest. Expected behavior for `--force`. ✓
- **No new debug prints, TODOs, commented-out code, or secrets.** ✓
- **No new exception swallowing.** `diskErr` is checked explicitly; `HashFile` errors fall through to safe paths. No `_` discards on new calls. ✓
- **Import / package surface unchanged.** `diff.go` still imports only `io/fs`, `path/filepath`, and `scaffold`. No new dependencies. ✓

### Cross-check against Round 4 follow-ups

- **LOW redundant `available` build under `apErr != nil`** — unchanged (untouched in this commit). Still non-blocking.
- **LOW `found` loop has no break in `TestRunUpgrade_SurvivesAvailablePacksFailure`** — unchanged.
- **LOW FS swap duplicates `setupTestEmbedFS` literal** — unchanged.

None of these interact with the current commit's scope.

### Coverage gaps (for `/test`, not blocking merge here)

- The reintroduction path is not exercised end-to-end through `runUpgrade` (only at the diff-engine unit level). A CLI-level test that forces the template through v1 → v2 → v3 would catch integration regressions where, for example, the `resolveConflict` stdin-closed path ("skip") + the empty-`OldHash` MEDIUM finding above combine into a visible same-version loop.
- The heal-conflict → skip → next-upgrade convergence is similarly not covered at CLI level. A two-phase `runUpgrade` test with `hash=""` + user edit + closed stdin would assert that the second invocation produces zero conflicts.
- None of the three new tests exercise `--force`. The force path writes `NewContent` on both conflict classes; a test would confirm no regression there.

### Recommendation

- Merge: **approve, with one MEDIUM follow-up recommended before the next release**. No CRITICAL or HIGH findings. The stop-condition for `/self-review` (CRITICAL finding) is not triggered.
- The MEDIUM finding (reintroduction `ActionConflict` missing `OldHash: newHash`) is a latent one-upgrade-delayed convergence bug that mirrors Finding 6 exactly. The fix is a one-line addition and a matching test assertion. Strongly recommended to ship in the same PR if a revision round is acceptable; otherwise suitable as an immediate follow-up.
- LOW findings (permission-error fall-through, `Ada:` typo, missing `OldHash`/`DiskHash` assertions in the two new tests) are cosmetic or defensive hardening that can ship separately.
- The Round 4 Codex P2 ACTION_REQUIREDs ("heal loop" / "reintroduction overwrite") are substantively resolved. The two fixes address real user-data-loss edge cases and are backed by focused unit tests. The commit is narrow, the diff is easy to audit, and no unrelated churn was introduced.
- No tech-debt entry required. The MEDIUM finding is small enough to fix inline rather than defer; the LOW permission-error path is a pre-existing pattern that can be addressed in a future dedicated hardening commit.

**Verdict: approve / no CRITICAL findings. Stop condition for `/self-review` not triggered. One MEDIUM follow-up recommended.**
