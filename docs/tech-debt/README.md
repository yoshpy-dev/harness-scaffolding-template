...
| --- | --- | --- | --- | --- |
| Exit code collision in `run_outer_loop`: `gh_unavailable` and codex `ACTION_REQUIRED` both return 1 | Missing `gh` CLI causes wasted Inner Loop cycles until `max_iterations` is reached | Preflight probe warns at startup; practical impact is low because `gh` is almost always present | If a user reports unexpected `max_iterations` exhaustion with `gh` CLI absent | `docs/reports/codex-triage-ralph-pipeline-hardening.md` finding #3, `docs/reports/self-review-2026-04-15-ralph-pipeline-hardening-v2.md` |
